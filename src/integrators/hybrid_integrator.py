...
try:
    ...
    DeviceManager = None
    are_drop_values_equivalent = None
...
class HybridIntegrator:
    ...
    def __init__(
        ...
        # --- NEW: Check for new imports ---
        if are_drop_values_equivalent is None or normalize_drop_number is None:
            raise RuntimeError("Ensemble helper functions failed to import.")
        ...
        if self.dataset_type != 'drop' and AlignmentLayer is not None:
            try:
                ...
                sym_dim = getattr(self.dim_manager, 'sym_dim', 384)
                neural_dim = getattr(self.dim_manager, 'neural_dim', 768)
        ...
        self.cache: Dict[str, Tuple[Any, str, float]] = {}
    ...
    def _fuse_symbolic_neural_for_drop(self,
        ...
        self.logger.debug(
            f"[DROP QID:{qid}] Starting fusion. "
            f"Symbolic status: {symbolic_result_obj.get('status')}, Neural input type: {type(neural_output_obj)}")

        # --- Configurable Fusion Thresholds ---
        STRONG_SYM_CONF_THRESHOLD = 0.80
        STRONG_NEU_CONF_THRESHOLD = 0.75
        MIN_USABLE_SYM_CONF = 0.35
        MIN_USABLE_NEU_CONF = 0.45
        ...
        sym_number_val = normalize_drop_number(symbolic_result_obj.get('number'))
        ...
        if isinstance(neural_output_obj, dict) and neural_output_obj.get('status') == 'success':
            ...
            effective_neu_value_type = neural_output_obj.get('type')
            neural_rationale = neural_output_obj.get('rationale', 'Neural parsed result from structured input')
            neu_number_val = normalize_drop_number(neural_output_obj.get('number'))
            ...
            if parsed_from_string:
                ...
                if effective_neu_value_type == 'number':
                    neu_number_val = normalize_drop_number(parsed_value_content)
        ...
        # Case 3: Both symbolic and neural have usable results; compare them
            ...
            if effective_sym_value_type == effective_neu_value_type:
                ...
                values_agree_exactly = are_drop_values_equivalent(sym_comp_dict, neu_comp_dict,
                ...
                if current_comparison_type == 'spans' and not values_agree_exactly and self.encoder and util:
                    if sym_spans_val and neu_spans_val:
                ...
                if sym_conf > neu_conf + CONF_DELTA_FOR_NEURAL_OVERRIDE_IF_TYPES_MISMATCH and sym_conf >= MIN_USABLE_SYM_CONF:
                    final_status, final_chosen_op_type, final_confidence, final_rationale = \
                    ...
                    if effective_sym_value_type == 'number':
                        ...
                        final_chosen_value_content = sym_date_val
                elif neu_conf > sym_conf + CONF_DELTA_FOR_NEURAL_OVERRIDE_IF_TYPES_MISMATCH and neu_conf >= MIN_USABLE_NEU_CONF:
                    final_status, final_chosen_op_type, final_confidence, final_rationale = \
                    ...
                    final_chosen_op_type = 'error'

        # Case 4: Only Neural is usable
        ...
        # Case 5: Only Symbolic is usable
        ...
        # Case 6: Fallback if no path is clearly superior or usable
            ...
            final_chosen_op_type = sym_op_type or effective_neu_value_type or 'error'
            final_confidence = 0.05
        ...
        # Add component timings to the debug info
        debug['symbolic_time'] = self.component_times.get('symbolic', 0.0)
        debug['neural_time'] = self.component_times.get('neural', 0.0)
